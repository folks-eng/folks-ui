...
<div class="modal-overlay" id="scheduleOverlay" aria-hidden="true">
  ...
  function goToAddressStep(direction) {
    const addresses = getAddresses();
    if (addresses.length === 0) {
      goTo(screens.addressForm, direction);
      return;
    }
    if (!addresses.some(a => a.id === state.selectedAddressId)) {
  ...
  function renderAddressChoices() {
    ...
    addressList.innerHTML = addresses.map(a => `
      <button type="button" class="address-choice${a.id === state.selectedAddressId ? ' is-active' : ''}" data-address-choice="${a.id}">
        <span class="address-choice-label">${escapeHtmlSched(a.label || 'Address')}</span>
        <span class="address-choice-summary">${escapeHtmlSched([a.line1, a.line2, a.city].filter(Boolean).join(', '))}</span>
      </button>
    `).join('');

    addressList.querySelectorAll('[data-address-choice]').forEach(btn => {
      btn.addEventListener('click', () => {
        state.selectedAddressId = btn.dataset.addressChoice;
        renderAddressChoices();
  ...
  addressConfirmBtn.addEventListener('click', () => {
    const address = getAddressById(state.selectedAddressId);
    if (!address) {
  ...
  document.getElementById('scheduleAddressFormSaveBtn').addEventListener('click', () => {
    ...
    if (!line1) return showAddressFormError('Please enter the primary address line.');
    if (!city) return showAddressFormError('Please enter a city.');
    if (!stateVal) return showAddressFormError('Please enter a state.');
    if (!/^[0-9A-Za-z\- ]{3,10}$/.test(postalCode)) return showAddressFormError('Please enter a valid postal code.');
