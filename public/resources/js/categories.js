...
document.addEventListener('DOMContentLoaded', () => {
  if (typeof CATEGORY_DATA === 'undefined') return; // guard: only runs on categories.html
  initCategoriesPage();
});
...
function initCategoriesPage() {
  ...
  if (!catPillRow || !skuGrid) return;

  const totalServiceCount = CATEGORY_DATA.reduce(
    (sum, cat) => sum + countServices(cat), 0
  );
  const statTotal = document.getElementById('catStatTotal');
  if (statTotal) statTotal.textContent = `${totalServiceCount} services across ${CATEGORY_DATA.length} categories`;

  const state = {
    activeCategoryId: CATEGORY_DATA[0].id,
  ...
  if (CATEGORY_DATA.some(c => c.id === hashId)) {
    state.activeCategoryId = hashId;
  }
  ...
  function renderCategoryPills() {
    ...
    catPillRow.innerHTML = CATEGORY_DATA.map(cat => `
      <button type="button" class="cat-pill${cat.id === state.activeCategoryId ? ' is-active' : ''}"
              role="tab" aria-selected="${cat.id === state.activeCategoryId}"
  ...
  function renderSubcategoryChips() {
    const category = CATEGORY_DATA.find(c => c.id === state.activeCategoryId);
    if (!category) return;

    const chips = [{ id: 'all', name: 'All', count: countServices(category) }]
      .concat(category.subCategories.map(sub => ({ id: sub.id, name: sub.name, count: sub.services.length })));
    ...
    subcatChipRow.querySelectorAll('.subcat-chip').forEach(btn => {
      btn.addEventListener('click', () => {
        state.activeSubCategoryId = btn.dataset.subcategoryId;
        renderSubcategoryChips();
        renderBanner();
        renderGrid();
      });
  ...
  function renderBanner() {
    ...
    const subCategory = state.activeSubCategoryId !== 'all'
      ? category.subCategories.find(s => s.id === state.activeSubCategoryId)
      : null;
  ...
  function renderGrid() {
    ...
    if (state.searchQuery) {
      ...
      category.subCategories.forEach(sub => {
        if (state.activeSubCategoryId !== 'all' && sub.id !== state.activeSubCategoryId) return;
        sub.services.forEach(sku => items.push({ sku, category, subCategory: sub }));
      });
      const subLabel = state.activeSubCategoryId !== 'all'
        ? ' · ' + escapeHtml(category.subCategories.find(s => s.id === state.activeSubCategoryId).name)
        : '';
  ...
  function renderSkuCard(sku, category, subCategory) {
    const cartItem = getCart().find(i => i.skuId === sku.id);
    const qty = cartItem ? cartItem.qty : 0;
    return `
      <article class="sku-card" data-sku-id="${sku.id}">
        <div class="sku-thumb">
          <img src="${subCategory.image}" alt="" loading="lazy">
          ${sku.isPopular ? '<span class="sku-popular-badge">Popular</span>' : ''}
          <button type="button" class="sku-wishlist-btn" data-wishlist-toggle aria-label="Save ${escapeHtml(sku.name)} to wishlist" aria-pressed="${isWishlisted(sku.id)}">
        ...
        <div class="sku-card-body">
          ...
          <div class="sku-meta-row">
            ...
            <span class="rating" aria-label="Rated ${sku.rating} out of 5">
          ...
          <div class="sku-footer">
            <span class="sku-price">${sku.currency}${sku.price.toLocaleString('en-IN')}<small><br>onward</small></span>
  ...
  function wireCardControls(items) {
    items.forEach(({ sku, category, subCategory }) => {
      const card = skuGrid.querySelector(`[data-sku-id="${sku.id}"]`);
      if (!card) return;
      ...
      incBtn.addEventListener('click', () => {
        ...
        const existing = getCart().find(i => i.skuId === sku.id);
        addToCart(sku, category, subCategory, existing);
        renderGrid();
      });

      decBtn.addEventListener('click', () => {
        removeFromCart(sku.id);
        renderGrid();
      });

      if (changeSlotBtn) {
        changeSlotBtn.addEventListener('click', () => {
          const current = getCart().find(i => i.skuId === sku.id);
          openSchedulePicker(sku.name, current, (schedule) => {
            updateCartSchedule(sku.id, schedule);
            renderGrid();
          });
        });
      }
  ...
  function addToCart(sku, category, subCategory, schedule) {
    const cart = getCart();
    let item = cart.find(i => i.skuId === sku.id);
    if (!item) {
      item = {
        skuId: sku.id,
        name: sku.name,
  ...
  function removeFromCart(skuId) {
    ...
    if (!item) return;
    item.qty -= 1;
    if (item.qty <= 0) cart = cart.filter(i => i.skuId !== skuId);
    saveCart(cart);
    updateCartBar();
  }
