...
document.addEventListener('DOMContentLoaded', () => {
  if (typeof getCurrentUser === 'undefined') return; // guard: only runs on this page
...
function initBecomeProfessionalPage() {
  ...
  if (!gate || !formContent) return;

  const showEl = (el) => { if (el) el.hidden = false; };
  const hideEl = (el) => { if (el) el.hidden = true; };

  if (!isLoggedIn() || !getCurrentUser()) {
    showEl(gate); hideEl(alreadyApplied); hideEl(formContent); hideEl(success);
  ...
  if (user.role === 'Professional' && user.professionalStatus) {
    hideEl(gate); hideEl(formContent); hideEl(success);
  ...
  hideEl(gate); hideEl(alreadyApplied); hideEl(success);
  showEl(formContent);
...
function renderExpertiseGroups() {
  ...
  container.innerHTML = CATEGORY_DATA.map(cat => `
