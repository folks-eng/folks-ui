...
document.addEventListener('DOMContentLoaded', () => {
  if (typeof getCart === 'undefined') return; // guard: only runs on checkout.html
  initCheckoutPage();
});
...
function initCheckoutPage() {
  ...
  if (!isLoggedIn() || !getCurrentUser()) {
    show(gate); hide(empty); hide(content); hide(success);
    const gateBtn = document.getElementById('checkoutGateSignupBtn');
    if (gateBtn) gateBtn.addEventListener('click', () => document.getElementById('signupBtn')?.click());
    return;
  }

  if (getCart().length === 0) {
    hide(gate); show(empty); hide(content); hide(success);
    return;
  }

  hide(gate); hide(empty); show(content); hide(success);
  renderOrderSummary();
...
function wireOrderLineControls() {
  document.querySelectorAll('[data-order-increment]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (item) { item.qty += 1; saveCart(cart); renderOrderSummary(); }
    });
  });
  ...
  document.querySelectorAll('[data-order-remove]').forEach(btn => {
    btn.addEventListener('click', () => {
      const skuId = btn.dataset.orderRemove;
  ...
  document.querySelectorAll('[data-order-change-slot]').forEach(btn => {
    btn.addEventListener('click', () => {
      const skuId = btn.dataset.orderChangeSlot;
      const cart = getCart();
      const item = cart.find(i => i.skuId === skuId);
      if (!item) return;
...
function wireCompleteBooking() {
  ...
  btn.addEventListener('click', async () => {
    ...
    const payload = {
      customer: user ? { id: user.id, name: user.name, email: user.email, mobile: user.mobile } : null,
...
function showBookingSuccess(booking, amount) {
  ...
  if (msg) msg.textContent = `Your booking is confirmed. A total of ₹${amount.toLocaleString('en-IN')} will be collected as agreed.`;
  if (idEl) idEl.textContent = booking && booking.id ? booking.id : '—';
  show(success);
...
function formatScheduleLong(item) {
  if (!item.date) return 'No slot selected';
