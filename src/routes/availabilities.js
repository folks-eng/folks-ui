    ...
    try {
        ...
        if (response.status === 200) {
            ...
            if (log.isDebugEnabled()) {
                log.debug('Successfully fetched availabilities. Response:\n%s', JSON.stringify(result, null, 2));
            }
            ...
            let result = response.data;
            log.error('Unable to fetch availabilities. Status code: %d. Error Msg: %s', response.status, result);
