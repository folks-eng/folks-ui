    ...
    try {
        ...
        if (response.status === 200) {
            ...
            if (log.isDebugEnabled()) {
                log.debug('Successfully fetched voucher details. Response:\n%s', JSON.stringify(result, null, 2));
            }
            ...
            let result = response.data;
            log.error('Unable to fetch voucher details. Status code: %d. Error Msg: %s', response.status, result);
        ...
        handleError(req, res, err, 'Error in fetching voucher details');
    }
}
